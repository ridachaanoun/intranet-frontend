...
const cursus = ref(null);

onMounted(async () => {
  await fetchCursusHistory();
});

async function fetchCursusHistory() {
  try {
    const response = await api.get(`user/cursus-history/${props.id}`);
    cursus.value =  await response.data.cursus_history
  } catch (error) {
    console.error('Failed to fetch cursus history:', error);
    cursus.value = [];
  }
}
